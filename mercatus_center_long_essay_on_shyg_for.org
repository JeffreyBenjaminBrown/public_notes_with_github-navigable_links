...
The extreme expressivity of the query language, and the extremely broad nature of the index, will permit a wide variety of desirable applications. In the next section I will sketch [[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#some-things-shyg-will-enable][some potential applications]], and in the process, introduce some of Shyg's key architectural ideas. Then I will describe [[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#an-overview-of-shygs-architecture][Shyg's architecture]]. After that I will examine Shyg's technical and economic [[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#feasibility][feasibility]], including open questions. Last, I will describe in the project's budget requirements. (And then there are footnotes.)

If the promises in the first section seem outlandish, perhaps consider skipping to the [[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#an-overview-of-shygs-architecture][architecture section]] to see how I plan to do it. And if the architectural overview seems too easy, perhaps consider skipping to the [[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#feasibility][feasibility section]], where I describe plainly the hard problems I foresee.
...
:END:
The last query mentioned will be made possible with the help of an ontology. Later this proposal describes the term [[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#the-ontology][in more detail]], but in brief, an ontology is a collection of relationships intended to help understand the world's categories.
...
That last component, [[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#the-hash-query-language][Hash]], I wrote myself, albeit without support for federated queries. I intend to rewrite it it in TypeQL, the query language for [[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#the-typedb-hypergraph-database][TypeDB]], an open-source hypergraph database. Unlike TypeQL, Hash resembles natural language.
...
As described above in "[[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#search-better][search better]]", inclusion of an ontology in (or, more accurately, continuously incorporating ontological relationships into) the Shyg index will allow users to search across large categories without having to specify every member of each category.
...
:END:
It will surely be a long time, if ever, before Shyg has indexed as many sites as Google has. Nevertheless, over the portion of the internet that Shyg's users choose to index (see "[[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#determining-what-to-index-and-in-how-much-detail][determining what to index, and in how much detail]]", below), Shyg will be able to answer queries that no other search engine can. This is because both the hypergraph index and the language for querying it will be more expressive than anything that has come before.
...
:END:
Every major part of Shyg's architecture will be a preexisting technology. The first subsection, [[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#well-established-tech-to-draw-on][well-established tech to draw on]], describes these. But there are some questions I have not yet solved regarding how to use those components, which I turn to in the last two subsections: [[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#unsolved-and-important-problems][unsolved and important problems]] and [[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#unsolved-optional-problems][unsolved optional problems]]. The hardest of these regard sharing: jointly determining what to index, distributing the index among users, and cooperating to process queries.
** well-established tech to draw on
:PROPERTIES:
:ID:       604eaacb-6780-441b-b1c7-6a3bc86660fe
:END:
*** TypeDB
TypeDB, the company, provides a powerful open-source hypergraph database, also called [[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#the-typedb-hypergraph-database][TypeDB]], and a query language for it called TypeQL. The query language is somewhat hostile to non-programmers. That is fine, because another part of Shyg's stack will be:
*** Hash
[[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#the-hash-query-language][Hash]] is an open-source, user-friendly query language resembling ordinary speech. I implemented it in Haskell a few years ago. I will rewrite it in TypeQL.
*** Emacs
...
** the path to growth
As mentioned earlier, [[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#sidenote-shyg-does-not-need-scale-in-order-to-be-useful][Shyg does not need scale in order to be useful]]. It will, however, require some proselytizing to build the initial user base. If they will find it sufficiently useful and/or fun, they will tell others.
...
Shyg plays to all three interests -- it is massively open and decentralized, but also allows people to keep separate private indices. Much of this audience is in search of a better social media experience. As described in [[https://github.com/JeffreyBenjaminBrown/public_notes_with_github-navigable_links/blob/master/mercatus_center_long_essay_on_shyg_for.org#build-your-own-social-media-feed][build your own social media feed]], Shyg could provide a unified, ad-free, psychologically non-manipulative interface for all sorts of online content, including but not limited to social media.
**** AI developers
